...
void Game::initializeGrid()
{
	...
	//
	// /\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\ //
	for (int iRow = 0; iRow < this->m_nGridRows; iRow++/* ?? complete for loop ?? */)
	{
		for (int iColumn = 0; iColumn < this->m_nGridColumns; iColumn++/* ?? complete for loop ?? */)
		{
			Game::a_cGrid[iRow][iColumn] = '~';/*?? assign '~' to element of 2-D array a_cGrid indexed by iRow and iColumn ?? */
		}
	}
	return;
}
...
void Game::displayGrid(bool bShowBoat) {
	...
	for (int iRow = 0; iRow < this->m_nGridRows; iRow++/* ?? complete for loop specification ?? */)
	{
		std::cout << "|";
		for (int iColumn = 0; iColumn < this->m_nGridColumns; iColumn++/* ?? complete for loop specification ?? */)
		{
...
void Game::placeBoat()
{
	...
	// /\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\ //
	this->a_cGrid[iRow][iColumn] = 'V';/* ?? Assign 'V' to a_cGrid at the appropriate row and column index ?? */
	this->m_iRowBoat = iRow;/* ?? Assign value to this->m_iRowBoat ?? */
	this->m_iRowColumn = iColumn;/* ?? Assign value to this->m_iRowColumn ?? */
	return;
}
...
bool Game::isCloseToTarget(int iRow, int iColumn)
{
	...
	// the target boat.
	// /\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\ //
	if (iColumn <= this->m_iColumnBoat+1 && iColumn >= this->m_iColumnBoat-1 && iRow <= this->m_iRowBoat+1 && iRow >= this->m_iRowBoat-1)
	{
		bClose = true;
	}
...
void Game::showSunk()
{
	...
	// /\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\ //
	this->a_cGrid[this->m_iRowBoat][this->m_iColumnBoat] = 'X'/* ?? Assign 'X' to a_cGrid at location (row/column) of Battle Boat ?? */
	return;
}
...
std::tuple<int, int> Game::promptForGridCoord()
{
	...
	// /\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\ //
  std::cout << "Please enter grid row number: " << std::endl;/* ?? Prompt user using std::cout for row ?? */
	std::cin >> iRow;/* ?? Use std::cin ?? */

  std::cout << "Please enter grid column number: " << std::endl;      /* ?? Prompt user using std::cout for column ?? */
	std::cin >> iColumn;/* ?? Use std::cin ?? */
...
void Game::play()
{
	...
	while ((!bSunk) && (this->m_nTorpedoes > 0))
	{
		...
		if (Game::isOnTarget(iRow, iColumn)/* ?? call a member function to check if Battle Boat has been hit; function returs a bool ?? */)
		{
			this->showSunk();
		...
		// /\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\ //
		if (bSunk == false/* ?? Boolean expression for not sunk */)
		{
			std::cout << "Magazine: " << this->m_nTorpedoes/* ?? remaining number of torpedoes ?? */ << " torpedoes" << std::endl;
		}
		...
		// /\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\ //
		if (this->m_nTorpedoes == 0 && bSunk == false/* ?? number of remaining torpedoes is 0 and Battle Boat is not sunk ?? */)
		{
			// Display grid shown boat's location
			Game::displayGrid(true);/* ?? call member function to dislay the grid and show the location of the battle boat ?? */
